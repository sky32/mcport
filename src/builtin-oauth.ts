...
function interactionPage(input: {
  ...
  const tierLabel = ({ readonly: '只读', standard: '标准', full: '完整' } as const)[input.toolTier];
  const permissions = [
    ...
    ...(input.toolTier === 'standard' || input.toolTier === 'full'
      ...
      : []),
    ...(input.toolTier === 'full'
      ? ['调用 Runtime 允许的本机命令；仍受 Allowed Commands、命令执行开关和超时限制']
      : []),
  ];
