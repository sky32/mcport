...
const indexPath = 'desktop/renderer/index.html';
...
for (const value of new Set(textChunks)) {
  if (!catalog.has(value)) failures.push(`HTML text is missing from i18n catalog: ${value}`);
}

for (const match of html.matchAll(/\b(?:placeholder|title|aria-label|data-help)="([^"]*[\u3400-\u9fff][^"]*)"/gu)) {
  const value = match[1].trim();
  if (!catalog.has(value)) failures.push(`HTML attribute is missing from i18n catalog: ${value}`);
}
...
for (const match of app.matchAll(/\bt\(\s*([^\n)]*)\)/gu)) {
  for (const keyMatch of match[1].matchAll(/['"]([\w-]+\.[\w.-]+)['"]/gu)) {
    if (!keyedNames.has(keyMatch[1])) failures.push(`Unknown i18n key referenced by app.js: ${keyMatch[1]}`);
  }
}
...
console.log(`Renderer i18n check passed (${new Set(textChunks).size} HTML text chunks checked)`);
